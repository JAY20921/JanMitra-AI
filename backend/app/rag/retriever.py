...
class Retriever:
    ...
    def __init__(self):
        ...
        self.temp_store_manager = get_temp_store()
    ...
    async def _retrieve_temp_docs(self, query: str, session_id: str) -> List[Document]:
        ...
        temp_retriever = self.temp_store_manager.get_vector_store(
            session_id
        ...
        try:
            ...
            logger.error(
                "Failed to retrieve from TempSessionStore: %s", e, exc_info=True
            )
            return []

    async def _retrieve_qdrant_docs(
        self, query: str, user_profile: Optional[UserProfile], top_k: int
    ...
    async def _retrieve_tavily_docs(self, query: str) -> List[Document]:
        logger.info(
            "Insufficient global documents found locally. Falling back to Tavily Live Search..."
        )
        try:
    ...
    def _content_hash(text: str) -> str:
        """Return a short SHA-256 hex digest used for deduplication."""
        return hashlib.sha256(text.encode("utf-8")).hexdigest()

    @staticmethod
    def _deduplicate(docs: List[Document], seen_hashes: Set[str]) -> List[Document]:
        """Filter out documents whose page_content has already been seen."""
        unique: List[Document] = []
        for doc in docs:
            h = Retriever._content_hash(doc.page_content)
    ...
    async def retrieve_documents(
        ...
        Deduplicates results across tiers to prevent wasting LLM tokens.
        """
        ...
        if session_id:
            ...
            all_results.extend(self._deduplicate(temp_results, seen_hashes))

        # --- Tier 2: Qdrant Official Local Knowledge Base ---
        qdrant_results, qdrant_failed = await self._retrieve_qdrant_docs(
            query, user_profile, top_k
        )
        all_results.extend(self._deduplicate(qdrant_results, seen_hashes))
        ...
        if not qdrant_failed and len(qdrant_results) < 2:
            ...
            all_results.extend(self._deduplicate(tavily_results, seen_hashes))
