        ...
        with self._lock:
            ...
            if not exists:
                if len(self._session_metadata) >= self.MAX_COLLECTIONS:
                    raise RuntimeError(
                        "Temporary storage is currently at capacity. Please try again later."
                    )

                logger.info(
                    "Creating temporary in-memory collection: %s", collection_name
                )
...
_temp_store_lock = Lock()


def get_temp_store() -> TempSessionStore:
    """Returns a lazily-initialized, reusable TempSessionStore singleton."""
