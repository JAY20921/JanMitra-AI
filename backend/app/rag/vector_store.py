...
class QdrantStore:
        ...
        try:
            ...
            try:
                ...
                if "force_recreate" in str(ve):
                    logger.warning(
                        "Dimension mismatch detected. Recreating Qdrant collection..."
                    )
                    self.vector_store = QdrantVectorStore(
                        client=self.client,
                        collection_name=self.collection_name,
    ...
    def get_vector_store(self) -> QdrantVectorStore:
        ...
        if self.vector_store is None:
            raise RuntimeError(
                "QdrantStore was not successfully initialized (database may be offline)."
            )
        return self.vector_store


# Module-level singleton pattern (thread-safe)
_qdrant_store_instance: Optional[QdrantStore] = None
_qdrant_store_lock = threading.Lock()
