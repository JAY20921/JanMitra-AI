...
def process_file_in_background(file_path: str):
    try:
        ...
        vector_store = get_qdrant_store().get_vector_store()
        vector_store.add_documents(chunks)
        logger.info(
            "Successfully processed and stored %d chunks for %s", len(chunks), file_path
        )
...
async def ingest_temp_document(
    ...
    try:
        ...
        logger.error(
            "Error processing temp file %s: %s", file.filename, e, exc_info=True
        )
        raise HTTPException(
            status_code=500, detail="Error processing document. Please try again."
        )
    finally:
