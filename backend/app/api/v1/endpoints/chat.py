    ...
    if request.stream:
        ...
        try:
            ...
            if (
                ...
                return ChatResponse(
                    response="*Note: I apologize, but the backend AI provider has exceeded its daily free-tier quota (API Rate Limit). Please switch API keys or try again tomorrow!*"
                )
            logger.error("Unexpected chat error: %s", e, exc_info=True)
            return ChatResponse(
                response="*Error: An unexpected issue occurred. Please try again later.*"
            )
