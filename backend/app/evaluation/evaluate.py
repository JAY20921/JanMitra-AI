...
async def evaluate_query(query_data, retriever, generator, evaluator_llm):
    ...
    # 2. Generate Answer
    answer = await generator.generate_response(query, user_profile, session_id="eval-session")
...
async def run_evaluation():
    dataset_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "data", "eval_dataset.json")
    ...
    for item in dataset:
        ...
        print(f"Scores -> Context: {c_score}/5 | Faithfulness: {f_score}/5 | Answer: {a_score}/5")
    ...
    report_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "eval_report.md")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# RAG Evaluation Report\n\n")
        f.write("## Aggregate Metrics\n")
        f.write(f"- **Average Context Relevance:** {avg_context:.2f} / 5.0\n")
        f.write(f"- **Average Faithfulness (Zero Hallucinations):** {avg_faithfulness:.2f} / 5.0\n")
